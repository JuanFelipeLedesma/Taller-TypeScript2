...
function renderTable(): void {
    ...
    if (!tableBody) return; // Asegúrate de que el cuerpo de la tabla existe
...
function displayDetails(id: number): void {
    console.log(`Mostrando detalles para: ${id}`); 
    const serie = series.find(s => s.id === id);
    const detailContainer = document.getElementById('series-detail') as HTMLElement;
